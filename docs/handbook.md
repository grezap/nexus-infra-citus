...
| T7 | Cold-rebuild `citus_extension` fails on a worker: `ERROR: cannot execute CREATE EXTENSION in a read-only transaction` (all 3 Patroni scopes had converged). | The extension overlay runs immediately after keepalived; on a fast from-zero rebuild the worker VIP hadn't yet settled on the Patroni **leader** (keepalived's `vrrp_script` needs an interval or two), so the connection transiently hit a **read-only replica**. (Did not surface in the live re-apply, where the VIPs had been bound for minutes.) | Add a `wait_rw` gate in the extension overlay: before wiring each worker, poll the VIP until `SELECT pg_is_in_recovery()` = `f` (VIP on a read-write primary), 40×3s. |
